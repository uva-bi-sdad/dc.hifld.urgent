...
#working directory
```{r}
setwd("~/VDH/Floating_Catchment_Areas/dmv/7. urgent_dmv_model/")
```
# read data
```{r}
...
plot_2sfca <- map %>%
  addControl( paste( specialty, "Per 1,000 People (2-Step Floating Catchment Area)") , "topright") %>%
...
plot_e2sfca <- map %>%
  addControl(paste( specialty, "Per 1,000 People (Enhanced 2-Step Floating Catchment Area)") , "topright") %>%
...
plot_3sfca <- map %>%
  addControl( paste( specialty, "Per 1,000 People (3-Step Floating Catchment Area)") , "topright") %>%
...
```{r}
dmv_bg_2021_acccess_scores_urgent <- rbind(dmv_bg_2021_acccess_scores_bg, 
...
#change one name
```{r}
dmv_bg_2021_acccess_scores_urgent <- read.csv("dmv_bg_2021_acccess_scores_urgent")

dmv_bg_2021_acccess_scores_urgent$measure <- ifelse(dmv_bg_2021_acccess_scores_urgent$measure=='urgent_pop', 'urgent_pop_cnt' , dmv_bg_2021_acccess_scores_urgent$measure)
```
...
write.csv(dmv_bg_2021_acccess_scores_urgent, "dmv_bg_2021_acccess_scores_urgent", row.names = FALSE )
...
sf::st_write(
  dmv_bg_2021_acccess_scores_urgent, con,
  c("dc_health_behavior_diet", "dmv_cttrbg_hifld_2021_acccess_scores_urgent"),
